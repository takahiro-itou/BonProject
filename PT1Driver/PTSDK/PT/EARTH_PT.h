...
namespace PT {
	...
	public:
		...
		// [機能] ソフトウェアバージョンを取得
		// [説明] バージョンが 1.2.3 の場合、値は 0x123 になります。
		//        上位 24 ビットが同じであればバイナリ互換になるように努めますので、
		//        ((version >> 8) == 1) であるかをチェックしてください。 
		virtual status GetVersion(uint *version) const = 0;

		// デバイス情報
		struct DeviceInfo {
			uint Bus, Slot, Function;	// PCI バス上の位置
			uint BadBitCount;			// PCI バスのビット化け数
		};

		// [機能] 認識されているデバイスのリストを取得
		// [説明] PCI バスをスキャンして以下の条件を全て満たすデバイスをリストアップします。
		//        ベンダID: 0x10ee / デバイスID: 0x211a / サブシステムベンダID: ~0x10ee / サブシステムID: ~0x211a
		//        
	...
	public:
		...
		// [機能] チューナー省電力制御
		// [説明] 復調IC と衛星側 PLL-IC のみが対象です。チューナーユニット内の他の回路は SetTunerPowerReset(TUNER_POWER_OFF)
		...
		// [機能] C/N と AGC を取得
		// [説明] C/N は短レイテンシで測定できるため、アンテナの向きを調整するのに便利です。
		//        値の意味は次の通りです。
		//        C/N                : cn100/100 (dB)
	...
	enum Status {
		...
		// バスクラスのエラー
		STATUS_WDAPI_LOAD_ERROR = (2)*256,	// wdapi921.dll がロードできない
